...
public class HugeInteger {
    ...
    public HugeInteger add(HugeInteger h){        
        ...
        if(this.isPos() && h.isNeg()){//h negative proceed with caution
            if(bigger == 1 || bigger == 0){
                return HugeInteger.subtract(this, h);
            }
            HugeInteger n = HugeInteger.subtract(h, this);
            n.positive = false;
            return n;
        }
        if(this.isNeg() && h.isPos()){//this negative proceed with caution
            if(bigger == 1){
                HugeInteger n  = HugeInteger.subtract(this, h);
                n.positive = false;
                return n;
            }
            return HugeInteger.subtract(h, this);
        }
        return null;
    }    
    private static HugeInteger add(HugeInteger a, HugeInteger b){
        String sum = "";
    ...
    public HugeInteger subtract(HugeInteger h){
        ...
        if(this.isPos() && h.isPos()){//both positive
            ...
            HugeInteger n = HugeInteger.subtract(h,this);//swap order
            n.positive = false;
            return n;
        }   
        return null;
    }
    private static HugeInteger subtract(HugeInteger a, HugeInteger b){
    ...
    public HugeInteger multiply(HugeInteger h){
        ...
        for(int i=0;i<this.length;i++){
            ...
            product = HugeInteger.add(new HugeInteger(HugeInteger.removeLeadingZeroes(partialProduct)), product);            
        }
